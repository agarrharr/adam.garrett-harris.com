...
module.exports = function (program, directory, suppliedStage) {
  ...
  function plugins(config) {
    config.plugin('define', _webpack2.default.DefinePlugin, [{
      'process.env': {
    ...
    switch (stage) {
      case 'develop':
        config.plugin('optimize-order', _webpack2.default.optimize.OccurenceOrderPlugin, null);
        config.plugin('hot-module-replacement', _webpack2.default.HotModuleReplacementPlugin, null);
        config.plugin('no-errors', _webpack2.default.NoErrorsPlugin, null);

        return config;
      case 'build-css':
        config.plugin('extract-text', _extractTextWebpackPlugin2.default, ['styles.css']);

        return config;
      case 'build-html':
        config.plugin('static-site-generator', _staticSiteGeneratorWebpackPlugin2.default, ['render-page.js', routes]);
        config.plugin('extract-text', _extractTextWebpackPlugin2.default, ['styles.css']);

        return config;
      case 'build-javascript':
        config.plugin('ignore', _webpack2.default.IgnorePlugin, [/^\.\/locale$/, /moment$/]);
        config.plugin('extract-text', _extractTextWebpackPlugin2.default, ['styles.css']);
        config.plugin('dedupe', _webpack2.default.optimize.DedupePlugin, null);
        config.plugin('uglify', _webpack2.default.optimize.UglifyJsPlugin, null);
    ...
    switch (stage) {
      case 'develop':
        ...
        config.merge({
          postcss: function postcss(wp) {
            return [require('postcss-import')({ addDependencyTo: wp }), require('postcss-cssnext')(), require('postcss-browser-reporter'), require('postcss-reporter')];
          }
        });
        return config;
      ...
      case 'build-css':
        ...
        config.loader('less', {
          test: /\.less/,
          exclude: /\.module\.less$/,
          loader: _extractTextWebpackPlugin2.default.extract(['css?minimize', 'postcss', 'less'])
        });
        config.loader('sass', {
          test: /\.(sass|scss)/,
          exclude: /\.module\.(sass|scss)$/,
